...
public class DiskCache implements Cache {
	...
	public synchronized void initialize() {
		if (!mRootDirectory.exists()) {
			if (!mRootDirectory.mkdirs()) {
				KJLoger.debug("Unable to create cache dir %s", mRootDirectory.getAbsolutePath());
			}
			return;
		}
	...
	public synchronized void put(String key, Entry entry) {
		...
		try {
			...
			if (!success) {
				fos.close();
				KJLoger.debug("Failed to write header for %s", file.getAbsolutePath());
	...
	public synchronized void remove(String key) {
		...
		removeEntry(key);
		if (!deleted) {
			KJLoger.debug("Could not delete cache entry for key=%s, filename=%s", key, getFilenameForKey(key));
		}
	}
	...
	private void pruneIfNeeded(int neededSpace) {
		...
		while (iterator.hasNext()) {
			...
			if (deleted) {
				...
				KJLoger.debug("Could not delete cache entry for key=%s, filename=%s", e.key, getFilenameForKey(e.key));
			}
			iterator.remove();
			prunedFiles++;
		...
		if (HttpConfig.DEBUG) {
			KJLoger.debug("pruned %d files, %d bytes, %d ms", prunedFiles, (mTotalSize - before), SystemClock.elapsedRealtime() - startTime);
		}
	}
