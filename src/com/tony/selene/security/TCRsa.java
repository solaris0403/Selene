package com.tony.selene.security;
...
public class TCRsa {
	...
	/**
	 * 
	 * RSA 公钥加密. 用公钥对一个字串进行加密然后用私钥解密,是RSA加密,只能加密少量数据.一般是用来加密会话密钥
	 * 
	 * @param content
	 *            内容
	 * @param publicKey
	...
	public static String encrypt(String content, PublicKey publicKey) {
		try {
			...
			String s = new String(TCBase64.encode(output));
			return s;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
	...
	/**
	 * 
	 * 获取数字签名. 利用私钥对一个字串进行加密,得到数字签名.是用来证明信息的确来自密钥所有人.然后用公钥解密。
	 * 
	 * @param content
	 *            内容
	 * @param privateKey
	...
	public static String sign(String content, String privateKey) {
		...
		try {
			PKCS8EncodedKeySpec priPKCS8 = new PKCS8EncodedKeySpec(TCBase64.decode(privateKey));
	...
	public static boolean doCheck(String content, String sign, String publicKey) {
		try {
			...
			byte[] encodedKey = TCBase64.decode(publicKey);
			...
			boolean bverify = signature.verify(TCBase64.decode(sign));
			return bverify;
	...
	private static PublicKey getPublicKeyFromX509(String algorithm, String bysKey) throws NoSuchAlgorithmException, Exception {
		byte[] decodedKey = TCBase64.decode(bysKey);
	 ...
	 * @throws Exception
	 */
	private static PrivateKey getPrivateKeyFromX509(String algorithm, String bysKey) throws NoSuchAlgorithmException, Exception {
		byte[] decodedKey = TCBase64.decode(bysKey);
		X509EncodedKeySpec x509 = new X509EncodedKeySpec(decodedKey);
	...
	public static PublicKey loadPublicKey(String publicKey) throws Exception {
		try {
			byte[] buffer = TCBase64.decode(publicKey);
			KeyFactory keyFactory = KeyFactory.getInstance(ALGORITHM);
	...
	public static PrivateKey loadPrivateKey(String privateKey) throws Exception {
		try {
			byte[] buffer = TCBase64.decode(privateKey);
			// X509EncodedKeySpec keySpec = new X509EncodedKeySpec(buffer);
